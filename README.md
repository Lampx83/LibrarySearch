...
- **frontend**: Build Vite → nginx, cổng **8019**.
- **backend**: FastAPI, cổng **8020**. **Dữ liệu Excel/CSV đã nhúng** trong repo (`backend/data/`), được copy vào image tại `/data` — deploy xong là dùng được, không cần mount volume.
...
3. Chọn **Git repository**:
   ...
   - **Branch**: `main`
4. (Tùy chọn) **Environment variables**: `HTTP_PORT` (8019), `BACKEND_PORT` (8020).
5. **Deploy the stack**. Dữ liệu đã nằm trong image, không cần cấu hình thêm.

### Dữ liệu Excel/CSV (tùy chọn)

**Mặc định:** Dữ liệu nằm trong repo (`backend/data/`) và được đóng gói vào image. Deploy từ Git là có sẵn, không bị 503.

**Muốn dùng thư mục khác trên host:** Sửa `docker-compose.yml`, bật lại `volumes` của backend và mount thư mục chứa file vào `/data`:
...
### Lỗi 503 khi tìm kiếm

Chỉ xảy ra nếu backend không đọc được file nào (vd. bạn mount volume trống lên `/data`). Deploy mặc định (dữ liệu nhúng trong image) thì không bị 503. Nếu đã bật volume: kiểm tra thư mục host có file, hoặc tắt volume để dùng dữ liệu nhúng.
